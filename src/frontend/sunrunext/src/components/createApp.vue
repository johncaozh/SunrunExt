<template>
  <el-container>
    ...
    <el-main>
      <el-form :model="form" ref="form" class="creatApp">
        <el-form-item label="应用Logo" :rules="[ { required: true, message: '产品Logo不能为空'}] " prop="logoUrl">
          <div class="avatar-container">
            <el-upload class="avatar-uploader" style="border: 1px dashed #d9d9d9;height:60px" :action="uploadUrl"
        ...
        <el-form-item label="应用介绍（选填）" :rules="[ { required: false, message: '产品描述不能为空'}] " prop="desc">
          <el-input v-model="form.desc" auto-complete="off" type="textarea"></el-input>
        </el-form-item>
        <el-form-item label="应用模式：" :rules="[ { required: true, message: '产品描述不能为空'}] " prop="useInGroup">
          <div style="width:100%">
            <el-radio v-model="form.useInGroup" label=false>普通应用</el-radio>
            <el-radio v-model="form.useInGroup" label=true>群应用</el-radio>
          </div>
        </el-form-item>
        <el-button type="primary" class="commit" @click="submitForm">创建应用</el-button>
      </el-form>
    </el-main>
  </el-container>
</template>
