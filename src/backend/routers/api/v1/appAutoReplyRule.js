...
router.get("/appAutoReplyRules", api.catchAsyncErrors(async function (req, res, next) {
    var foundData = await appAutoReplyRuleModel.find();
    api.attachData2Response(200, "获取成功", foundData, res);
    next();
}));

router.post("/appAutoReplyRules", api.catchAsyncErrors(async function (req, res, next) {
    var createdData = await appAutoReplyRuleModel.create(req.body);
    api.attachData2Response(200, "创建成功", createdData, res);
    next();
}));

router.get("/appAutoReplyRules/:id", api.catchAsyncErrors(async function (req, res, next) {
...
router.put("/appAutoReplyRules/:id", api.catchAsyncErrors(async function (req, res, next) {
    var id = req.params.id;
...
router.delete("/appAutoReplyRules/:id", api.catchAsyncErrors(async function (req, res, next) {
    var id = req.params.id;
